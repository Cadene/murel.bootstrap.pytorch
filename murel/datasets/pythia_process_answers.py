...
def process_answers(train_annotation_file, val_annotation_file=None, min_freq=9):

    # answer_file_name = "answers_vqa.txt"
    # os.makedirs(out_dir, exist_ok=True)
    Logger()('Processing answers BEGIN')
    train_answers = json.load(open(train_annotation_file, "r"))["annotations"]
    answers = train_answers

    if val_annotation_file is not None:
    ...
    aid_to_ans = answer_list
    ans_to_aid = {ans: i for i, ans in enumerate(aid_to_ans)}
    Logger()('Processing answers END')
    return aid_to_ans, ans_to_aid
