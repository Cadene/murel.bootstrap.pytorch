...
class VQA2(AbstractVQA):
    ...
    def add_rcnn_to_item(self, item):
        ...
        max_nb_regions = Options()['dataset'].get('max_nb_regions', None)
        if max_nb_regions is not None and \
           item['nb_regions'] > max_nb_regions:
            _, indices = item_rcnn['cls_scores'].sort(dim=0, descending=True)
            item['norm_coord'] = item['norm_coord'][indices[:max_nb_regions],:]
            item['coord'] = item['coord'][indices[:max_nb_regions],:]
            item['visual'] = item['visual'][indices[:max_nb_regions],:]
            item['nb_regions'] = max_nb_regions

        return item

    def bootstrapping(self):
        if Options()['dataset'].get('use_minival', False):
            #breakpoint()
            qid_minival = set(np.load('data/vqa/pythia/ids_minival.npy'))
            indices = [i for i, q in enumerate(self.dataset['questions']) if q['question_id'] in qid_minival]
            if self.split == "train":
